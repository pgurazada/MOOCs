...
mooc_df %>% 
  ...
  mutate(engaged = ifelse(viewed == 1 | explored == 1 | certified == 1, 1, 0),
         ...
         country = factor((final_cc_cname_DI)),
         education = factor(make.names(LoE_DI))) %>% 
...
X_test <- cb22x_neng_df %>% slice(-tr_rows) %>% select(-engaged) 
y_test <- cb22x_neng_df %>% slice(-tr_rows) %>% select(engaged) %>% mutate(engaged = factor(engaged)) %>% as_vector()
...
cb22x_neng_rf <- train(X_train, y_train,
                       method = "ranger",
                       tuneGrid = expand.grid(.mtry = 2:4, .splitrule = "gini", .min.node.size = c(10, 20, 30)),
                       trControl = trainControl(method = "repeatedcv",
                                               number = 10,
                                               repeats = 5),
