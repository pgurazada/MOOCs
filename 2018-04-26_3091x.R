...
mooc_df %>% 
  filter(course_id == "MITx/3.091x/2012_Fall") %>% 
...
#' MIT 3.091x is an introduction to solid state chemistry. 
...
mit3091x_df %>% 
  ...
  labs(x = "Engaged?",
       y = "Proportion",
       title = "Distribution of registered participants (MIT 3.091x)",
       caption = "Note: a registered participant engaged if they watched at least one video")

#' The above plot shows 50-50 split between the engaged and non-engaged 
#' participants 
...
mit3091x_df %>% 
  ...
  labs(x = "Engaged?",
       y = "Count",
       title = "Distribution of registered participants by gender (MIT 3.091x)") -> p1
...
mit3091x_df %>% 
  ...
  labs(x = "Engaged?",
       y = "Count",
       title = "Distribution of registered participants by level of education (MIT 3.091x)") -> p2
...
mooc_df %>% 
  ...
  mutate(engaged = ifelse(viewed == 1 | explored == 1 | certified == 1, 1, 0),
         launch_date = as.Date("2013-02-05"),
...
glimpse(mit3091x_neng_df)

#' We fit a couple of models to see if we can predict engagement from
#' demographics and registration times

tr_rows <- createDataPartition(mit3091x_neng_df$engaged, p = 0.8, list = FALSE)

mit3091x_train <- mit3091x_neng_df[tr_rows, ]
mit3091x_test <- mit3091x_neng_df[-tr_rows, ]

mit3091x_neng_logit <- train(factor(engaged) ~ .,
                             data = mit3091x_train,
                             method = "glm",
                             metric = "Kappa",
...
summary(mit3091x_neng_logit)

mit3091x_neng_logit$results

confusionMatrix(mit3091x_neng_logit)

#' The logit returns a 63% accuracy on initial dropout

mit3091x_neng_rf <- train(factor(engaged) ~ .,
                          data = mit3091x_train,
                          method = "rf",
...
mit3091x_neng_rf$results
varImp(mit3091x_neng_rf)
