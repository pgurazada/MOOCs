...
To begin with, lets look at the number of students enrolled for each course, and a summary of their activity. There are4 types of students. registered, i.e., all students in the data. 'viewed', i.e., logged in just one time. 'explored', i.e., viewed half or more chapters. 'certified', i.e., achieved passing grade. 
```{r}
mooc_df %>% 
  filter(is.na(incomplete_flag)) %>% 
  group_by(course_id) %>% 
  summarize_at(vars(registered, viewed, explored, certified),
...
Lets look at the number of unique users across the different courses. 
```{r}
mooc_df %>% 
  filter(is.na(incomplete_flag)) %>% 
  group_by(course_id) %>% 
...
length(unique(mooc_df$userid_DI))
```

Given the diversity in the course content, it would be ideal if we do not pool the data into one.

A good dependent variable is the number of days a student is active.

Let us run a typical analysis for the HarvardX, CB22x course:
...
cb22x_df %>% 
  ...
  mutate(active_for = last_event_DI - start_time_DI) %>% 
...
Lets see if we can fix the missing values. We use the K-nearest neighbors algorithm to fill them in.
```{r}
impute_na <- preProcess(cb22x_view_df, 
                        method = "knnImpute",
                        trControl = trainControl(method = "repectedcv",
                                                 number = 10, 
                                                 repeats = 5),
                        tuneGrid = data.frame(k = 2:10)) 

cb22x_clean_df <- predict(impute_na, cb22x_view_df)
...
```{r}
tr_rows <- createDataPartition(cb22x_view_df$viewed, p = 0.8, list = FALSE)
