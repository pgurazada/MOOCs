#' ---
#' title: "Analysis of MIT 6.002x/2013_Spring"
#' author: Pavan Gurazada
#' output: github_document
#' ---
#' last update: Wed Apr 25 16:10:35 2018
...
mooc_df %>% 
  filter(course_id == "MITx/6.002x/2013_Spring") %>% 
...
#' circuits (The same course was offered 2 consecutive times)
...
#' The above figure shows a 50-50 split between the notengaged-engaged
#' participants.  Did something change the second time the course was run?
...
mit6002x_df %>% 
  ...
  labs(x = "Engaged?",
       y = "Count",
       title = "Distribution of registered participants by gender (MIT 6.002x)") -> p1
...
mit6002x_df %>% 
  ...
  labs(x = "Engaged?",
       y = "Count",
       title = "Distribution of registered participants by level of education (MIT 6.002x)") -> p2
...
mooc_df %>% 
  ...
  mutate(engaged = ifelse(viewed == 1 | explored == 1 | certified == 1, 1, 0),
         launch_date = as.Date("2013-03-03"),
...
#' The logit returns a 55% accuracy on initial dropout just based on the
#' demographics
