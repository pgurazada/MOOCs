#' ---
#' title: "Analysis of MITx/6.00x/2012_Fall" 
#' author: Pavan Gurazada
#' output: github_document
#' ---
#' last update: Thu Apr 26 10:50:23 2018
...
mooc_df %>% 
  filter(course_id == "MITx/6.00x/2012_Fall") %>% 
...
#' MIT 3.091x is an introduction to computer science using Python. 
...
mit600x_df %>% 
  ...
  labs(x = "Engaged?",
       y = "Proportion",
       title = "Distribution of registered participants (MIT 6.00x)",
       caption = "Note: a registered participant engaged if they watched at least one video")

#' The above plot shows 40-60 split between the engaged and non-engaged 
#' participants 
...
mit600x_df %>% 
  ...
  labs(x = "Engaged?",
       y = "Count",
       title = "Distribution of registered participants by gender (MIT 6.00x)") -> p1
...
mit600x_df %>% 
  ...
  labs(x = "Engaged?",
       y = "Count",
       title = "Distribution of registered participants by level of education (MIT 6.00x)") -> p2
...
glimpse(mit600x_neng_df)

#' We fit a couple of models to see if we can predict engagement from
#' demographics and registration times

tr_rows <- createDataPartition(mit600x_neng_df$engaged, p = 0.8, list = FALSE)

mit600x_train <- mit600x_neng_df[tr_rows, ]
mit600x_test <- mit600x_neng_df[-tr_rows, ]

mit600x_neng_logit <- train(factor(engaged) ~ .,
                            data = mit600x_train,
                            method = "glm",
                            metric = "Kappa",
...
summary(mit600x_neng_logit)

mit600x_neng_logit$results

confusionMatrix(mit600x_neng_logit)

#' The logit returns a 66% accuracy on initial dropout

mit600x_neng_rf <- train(factor(engaged) ~ .,
                         data = mit600x_train,
                         method = "rf",
...
mit600x_neng_rf$results
varImp(mit600x_neng_rf)
