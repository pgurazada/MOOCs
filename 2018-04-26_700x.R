#' ---
#' title: "Analysis of MITx/7.00x/2013_Spring"
#' author: Pavan Gurazada
#' output: github_document
#' ---
#' last update: Thu Apr 26 11:51:34 2018
...
mooc_df %>% 
  filter(course_id == "MITx/7.00x/2013_Spring") %>% 
...
#' MIT 7.00x is an introduction to Biology - the secret of life
...
mit700x_df %>% 
  ...
  labs(x = "Engaged?",
       y = "Proportion",
       title = "Distribution of registered participants (MIT 7.00x)",
...
mit700x_df %>% 
  ...
  labs(x = "Engaged?",
       y = "Count",
       title = "Distribution of registered participants by gender (MIT 7.00x)") -> p1
...
mit700x_df %>% 
  ...
  labs(x = "Engaged?",
       y = "Count",
       title = "Distribution of registered participants by level of education (MIT 7.00x)") -> p2

grid.arrange(p1, p2, nrow = 2)

#' Surprisingly male dominant
...
mooc_df %>% 
  ...
  mutate(engaged = ifelse(viewed == 1 | explored == 1 | certified == 1, 1, 0),
         launch_date = as.Date("2013-03-05"),
...
glimpse(mit700x_neng_df)

#' We fit a couple of models to see if we can predict engagement from
#' demographics and registration times

tr_rows <- createDataPartition(mit700x_neng_df$engaged, p = 0.8, list = FALSE)

mit700x_train <- mit700x_neng_df[tr_rows, ]
mit700x_test <- mit700x_neng_df[-tr_rows, ]

mit700x_neng_logit <- train(factor(engaged) ~ .,
                            data = mit700x_train,
                            method = "glm",
                            metric = "Kappa",
...
summary(mit700x_neng_logit)

mit700x_neng_logit$results

confusionMatrix(mit700x_neng_logit)

mit700x_neng_rf <- train(factor(engaged) ~ .,
                         data = mit700x_train,
                         method = "rf",
...
mit700x_neng_rf$results
varImp(mit700x_neng_rf)

confusionMatrix(mit700x_neng_rf)
